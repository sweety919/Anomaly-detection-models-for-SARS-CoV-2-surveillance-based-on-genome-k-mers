...
def prepare_input_1(file_path):
    file = pd.read_csv(file_path)
    input_array = file.iloc[:,7:]
    input_array.fillna(0,inplace = True)
    input_array = np.array(input_array)
    target_list_ = file["classify_target"].values.tolist()
    target_list = []
...
if __name__ == "__main__":
    file_path = "D:/work files/stacking_model/divide_dataset/step2_dataset/"
    ecod_l = []
    ocsvm_l = []
    knn_l = []
    iforest_l = []
    ...
    lunar_l = []
    for i in range(1,6):
        train_path = file_path+"train_set_"+str(i)+".csv"
        test_path = file_path+"test_set_"+str(i)+".csv"
        train_set,train_target = prepare_input_1(train_path)
        test_set,test_target = prepare_input_1(test_path)
        print(i, "+1")
    ...
    ecod_f.to_csv("./step2_result/ecod.csv",index=False)
    ocsvm_f.to_csv("./step2_result/ocsvm.csv", index=False)
    knn_f.to_csv("./step2_result/knn.csv", index=False)
    iforest_f.to_csv("./step2_result/iforest.csv", index=False)
    ae_f.to_csv("./step2_result/ae.csv", index=False)
    lunar_f.to_csv("./step2_result/lunar.csv", index=False)
