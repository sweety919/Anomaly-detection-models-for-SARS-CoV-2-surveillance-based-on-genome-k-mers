...
def lunar_model_1(train_set,test_set):
    model = LUNAR()
    model.fit(train_set)
    pred_list = model.predict(test_set)
    return pred_list
...
if __name__ == "__main__":
    ...
    info_file = pd.read_csv("./重跑（仅VOC为positive）/pre_count_all(30-1).csv")
    whole_metrics = []
    knn_l = []
    for i in range(0,len(info_file)):
        ...
        if len(train_set_1)==0:
            ...
            if pred_list_1.tolist().count(1) == 0:
                ...
                print(i, "+2")
                pred_list_2 = lunar_model_1(train_set_2, test_set_2)
    ...
    whole_metrics_f.to_csv("./重跑（仅VOC为positive）/test_lunar+lunar1026.csv")
    knn_f = pd.DataFrame(knn_l)
    knn_f.to_csv("./重跑（仅VOC为positive）/step1_lunar(lunar+lunar)1026.csv")
    print(file)