...
def ocsvm_model(train_set,test_set,test_target):
    model = OCSVM(kernel='rbf',gamma='scale',nu=0.05,shrinking=True)
    model.fit(train_set)
...
if __name__ == "__main__":
    ...
    for i in range(len(info_file)):
        ...
        if len(train_set_1)==0:
            ...
            print(i, "+1")
            pred_list_1, metric_knn = ocsvm_model(train_set_1, test_set_1, test_target_1)
            ocsvm_l.append(metric_knn)
            # pred_list_1 = ocsvm_model(train_set_1, test_set_1)
    ...
    whole_metrics_f.to_csv("./test_ocsvm+lunar.csv",index=False)
    ocsvm_f = pd.DataFrame(ocsvm_l)
    ocsvm_f.to_csv("./step1_ocsvm(sklearn,kernel='rbf',gamma='scale',nu=0.05,shrinking=True).csv", index=False)
    print(file)